...
| | Code required? | Deterministic output? | Practitioner-owned? |
|---|---|---|---|
| Enterprise RPA | Yes | Yes | No — CoE builds it |
| AI Agents | No | No — inference-based | Partial |
| **GPA** | **No** | **Yes** | **Yes** |

GPA is the only cell in that matrix combining no-code implementation, deterministic execution, and practitioner ownership. That combination is not an accident. It is the specific answer to the specific gap the other two paradigms leave open.
